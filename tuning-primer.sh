...
## -- Key buffer Size -- ##
        ...
        if [ -n "$key_cache_miss_rate" ] && [ $key_cache_miss_rate -le 100 ] && [ $key_cache_miss_rate -gt 0 ] && [ -n "$key_buffer_freeRND" ] && [ $key_buffer_freeRND -le 20 ]; then
		...
		elif [ -n "$key_buffer_freeRND" ] && [ $key_buffer_freeRND -le 20 ] && [ -n "$key_buffer_size" ] && [ -n "$myisam_indexes" ] && [ $key_buffer_size -le $myisam_indexes ]; then
			cecho "You could increase key_buffer_size" boldred
			cecho "It is safe to raise this up to 1/4 of total system memory;"
			cecho "assuming this is a dedicated database server."
		elif [ -n "$key_cache_miss_rate" ] && [ $key_cache_miss_rate -ge 10000 ] || [ -n "$key_buffer_freeRND" ] && [ $key_buffer_freeRND -le 50 ]; then
